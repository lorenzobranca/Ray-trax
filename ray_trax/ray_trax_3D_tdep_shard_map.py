...
def trilinear_op(grid, x, y, z, value=None, mode="interp"):
    ...
    if mode == "interp":
        ...
        updates = jnp.zeros(grid.shape, dtype=grid.dtype)
        indices = [
...
def compute_radiation_field_from_source_with_time_step(
    ...
    #    J = jnp.zeros(j_map.shape, dtype=j_map.dtype)
    ...
    #        print("J shape inside loop:", J.shape)
    ...
    #        tau_new = tau + d_tau
    #        J_new = trilinear_op(J, x, y, z, value=I_new, mode="deposit")
    #        #J = jnp.asarray(J_new, dtype=J.dtype).reshape(J.shape)
    #        J = jax.lax.convert_element_type(J_new, J.dtype)
    ...
    def trace_single_ray(direction):
        ...
        def body_fn(i, state):
            ...
            return (
                jax.lax.stop_gradient(x_new),
                jax.lax.stop_gradient(y_new),
                jax.lax.stop_gradient(z_new),
                jax.lax.stop_gradient(I_new),
                jax.lax.stop_gradient(tau_new),
                jnp.asarray(J_updated, dtype=J.dtype).reshape(J.shape)
            )

        state_init = (x, y, z, I, tau, J)
        final_state = jax.lax.fori_loop(0, max_steps, body_fn, state_init)
        #_, final_state = jax.lax.scan(lambda c, _: (body_fn(0, c), None), state_init, None, length=max_steps)

        return final_state[-1]  # J
    ...
    if use_sharding:
        ...
        directions_reshaped = directions.reshape((n_devices, rays_per_device, 3))

        # Define mesh and sharding spec
        mesh = Mesh(np.array(devices), axis_names=('x',))
        input_sharding = P('x', None, None)  # [device, ray, 3]
        output_sharding = P('x', None, None, None, None)  # [device, ray, Nx, Ny, Nz]

        def batched_raytrace(dir_batch):
            return jax.vmap(trace_single_ray)(dir_batch)

        with mesh:
            J_all_sharded = shard_map(
                batched_raytrace,
                mesh = mesh,
...
def compute_radiation_field_from_multiple_sources_with_time_step(
    ...
    I_total = jnp.zeros(j_map.shape, dtype=j_map.dtype)
